...
> **v3 notice:** The `.cso` export has been **removed** from the Lychee Conductor v3. `cso-exporter.js`
...
The `.cso` (Cyberstar Online) format is a **custom** binary file format for Cyberstar show data. It stores pre-decoded frame bitmasks and music PCM in a compact layout. A decoder for this format was planned to be built into RR-Engine, but has not been implemented. **The `.cso` export was removed from the Lychee Conductor in v3.** This specification is preserved for reference only.
